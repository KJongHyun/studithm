...
import com.lightning.modules.Gathering.Avent.GatheringCreatedAvent;
...
public class GatheringService {
    ...
    public void updateGatheringDescription(Gathering gathering, GatheringDescriptionForm gatheringDescriptionForm) {
        ...
        eventPublisher.publishEvent(new GatheringUpdateAvent(gathering, "모임 소개를 수정했습니다."));
    }
    ...
    public void publish(Gathering gathering) {
        gathering.publish();
        this.eventPublisher.publishEvent(new GatheringCreatedAvent(gathering));
    }

    public void close(Gathering gathering) {
        gathering.close();
        eventPublisher.publishEvent(new GatheringUpdateAvent(gathering, "모임을 종료했습니다."));
    }

    public void startRecruit(Gathering gathering) {
        gathering.startRecruit();
        eventPublisher.publishEvent(new GatheringUpdateAvent(gathering, "팀원 모집을 시작합니다."));
    }

    public void stopRecruit(Gathering gathering) {
        gathering.stopRecruit();
        eventPublisher.publishEvent(new GatheringUpdateAvent(gathering, "팀원 모집을 중단했습니다."));
    }
